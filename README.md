...
**Milestone 1 — walking skeleton (complete).** Create a notebook, paste a source, pick a provider, and watch a grounded answer stream into chat; stop/interrupt, reload persistence, stub-provider E2E, and the required live-provider verification are complete. The current app also includes file upload with reviewable conversion to Markdown (`.md`, `.txt`, PDF, HTML, and SillyTavern lorebook/character-card JSON), editing a saved source after ingestion, native Preset Studio controls, per-exchange prompt inspection, review-before-save response capture, an optional "thinking" toggle with regenerate-as-swipeable-variants, bulk source selection, and a creative skills library with a one-click starter set attached per chat. The app is an installable PWA and the server serves the built web app single-origin in production. URL acquisition, search-backed organization, and richer update/diff/export workflows remain later [roadmap](docs/ROADMAP.md) work.
