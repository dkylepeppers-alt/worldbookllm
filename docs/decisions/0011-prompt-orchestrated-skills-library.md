...
6. **A curated starter set ships bundled.** Self-contained fiction skills from jwynia/agent-skills are vendored verbatim into the repo with the upstream MIT license text and an attribution manifest (source repo, commit, per-skill list), installable idempotently from the UI. Import-by-URL waits for M2's URL acquisition; MIT-in-AGPL bundling is compatible.
...
- The bundled starter set adds MIT-licensed third-party text to the repository, with attribution tracked in-tree; updates are manual re-vendoring.
